...
def html(content):
    """Renders a multi-line HTML string via st.markdown, safely stripping
    common leading indentation first (Streamlit's markdown renderer treats
    4+ leading spaces as a code block per standard Markdown rules, which
    silently breaks HTML rendering and shows raw tags as visible text)."""
...
# ----------------------------------------------------------------------
# NOTE: every custom HTML block below is written flush-left (no leading
# indentation on each line) before being passed to st.markdown. Streamlit's
# markdown renderer treats 4+ leading spaces as a code block per standard
# Markdown rules, which silently breaks HTML rendering and shows raw tags
# as visible text - flush-left avoids that entirely.
st.markdown("""
<style>
...
--surface-2: #242A2C;
--primary: #2FA98C;
--primary-dim: rgba(47,169,140,0.14);
--gold: #C9A227;
--alert: #E2665C;
--alert-dim: rgba(226,102,92,0.14);
--safe: #4FBE8C;
--safe-dim: rgba(79,190,140,0.14);
--line: #2B3133;
}
...
.sf-sidebar-title { font-family: 'Fraunces', serif; font-weight: 600; font-size: 1.25rem; margin: 0; color: var(--ink); }
...
.sf-alert-danger { background: var(--alert-dim); border: 1px solid rgba(226,102,92,0.35); }
.sf-alert-safe { background: var(--safe-dim); border: 1px solid rgba(79,190,140,0.35); }
...
background: var(--primary-dim); border: 1px solid rgba(47,169,140,0.3); border-radius: 10px; margin: 0.7rem 0;
}
...
.sf-result-conf { font-family: 'IBM Plex Mono', monospace; font-size: 0.95rem; color: var(--gold); margin-left: auto; }
...
[data-testid="stFileUploaderDropzone"] { border-radius: 10px; background: var(--surface); border: 1px solid var(--line); }
[data-testid="stFileUploaderDropzone"] button { background: var(--surface-2); color: var(--ink); border: 1px solid var(--line); }
...
# ----------------------------------------------------------------------
with st.sidebar:
    html('<div class="sf-badge">🚨</div>')
    html('<p class="sf-sidebar-title">SafeFall AI</p>')
    html('<p class="sf-sidebar-sub">CareVision HealthTech Pvt. Ltd.</p>')
    st.markdown("---")
...
<svg class="sf-pulse" viewBox="0 0 600 34" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
  <polyline points="0,17 140,17 160,17 172,4 184,30 196,10 208,17 240,17 600,17"
    fill="none" stroke="#C9A227" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
"""
...
# ----------------------------------------------------------------------
with col_main:
    html('<div class="sf-section-label">Input</div>')
...
# ----------------------------------------------------------------------
with col_dashboard:
    html('<div class="sf-section-label">Monitoring analytics</div>')
    ...
    if st.session_state.activity_log:
        ...
        html('<div class="sf-section-label">Activity distribution</div>')
        st.bar_chart(log_df["activity"].value_counts(), color="#2FA98C")

        html('<div class="sf-section-label">Recent predictions</div>')
